...
function initFormWithData() {
  ...
  // Copy initial data and convert date format
  props.headers.forEach(h => {
    if (props.initialData && props.initialData[h] !== undefined) {
      let value = props.initialData[h];
      
      // Convert ISO date to display format
      if (h === 'Дата' && typeof value === 'string' && value.includes('-')) {
  ...
  // Parse amount/sum field
  if (props.initialData && props.initialData['Сумма']) {
    const sumStr = String(props.initialData['Сумма']);
    if (sumStr.startsWith('$')) {
      amountCurrency.value = '$';
      amountValue.value = sumStr.replace(/[^\d.,]/g, '');
    } else {
  ...
  if (props.initialData && props.initialData['Количество']) {
    const qtyStr = String(props.initialData['Количество']);
    const match = qtyStr.match(/^([\d.,]+)\s*(кг|шт)?$/i);
    if (match && match[1]) {
      quantityValue.value = match[1];
...
function handleSubmit() {
  ...
  if (props.initialData?.original) {
    payload.original = props.initialData.original;
  }
